...
How to extract a user using the Go interface:
```go
package main
...
func main() {
  result, err := sear.Sear(map[string]any{
    "operation":  "extract",
    "admin_type": "user",
    "userid":     "IBMUSER",
  }, false)
...
The Go package lives in `go/sear` and can always be unit-tested with:
```shell
cd go
go test ./...
```

To compile with the native SEAR bridge enabled, build with the `sear_native` tag and link against the SEAR C library generated by CMake.
...
To enable the Go interface helper target in CMake:
```shell
cmake --preset <preset> -DSEAR_ENABLE_GO=on
cmake --build --preset <preset> --target go-sear
```
